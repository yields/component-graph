...
page('/:u/:c', function(ctx, next){
  ...
  g.on('node', function(node){
    ...
    nodes
      ...
      .style('fill', d3.hsl(hue, .7, .725))
      .style('stroke', d3.hsl(hue, .6, .625))
  ...
  g.on('built', function(){
    ...
    nodes.on('mouseover', function(n){
      d3.select(this).style('fill', d3.hsl(hue, .9, .9));
      d3.selectAll(children(n)).style('fill', d3.hsl(hue, .9, .9));
    ...
    // hide title
    nodes.on('mouseout', function(n){
      d3.select(this).style('fill', d3.hsl(hue, .7, .7));
      d3.selectAll(children(n)).style('fill', d3.hsl(hue, .6, .6));
